...
async function main() {
  console.log('🌱 Seeding ZOBBRA Staging Database (Clean Architecture & Catalog)...');

  // Clean existing tables in staging
  ...
  const acmeCompany = await prisma.company.create({
    data: {
      ...
      state: 'Odisha',
      pincode: '751024',
      notes: 'Enterprise merchandise client',
    },
  });

  // 2. Create Core System Users
  ...
  // 4. Create Products, Variants & Pricing Matrices
  ...
  // 5. Seed CMS Content (Testimonials, FAQs, Blogs)
  await prisma.cMSContent.createMany({
    data: [
      {
        type: 'TESTIMONIAL',
        title: 'Outstanding Quality & Speedy Turnaround!',
        content: 'ZOBBRA delivered 500 customized polo t-shirts and hoodies for our annual conference within 5 working days. High-quality DTF printing and neat embroidery.',
  ...
  await prisma.systemSetting.createMany({
    data: [
      { key: 'COMPANY_NAME', value: JSON.stringify('ZOBBRA Prints & Merchandise') },
  ...
  console.log('✅ ZOBBRA Staging Database Clean Seeding Complete!');
}
